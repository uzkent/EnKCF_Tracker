...
pixels and 10-20\% on AUC. \textit{YoungWoo : Two preceeding sentences start with
experimental results showed that. Should we change the begining of the second sentence?}
In addition, our implementation ran at 340
fps for the OTB100 and at 416 fps for the UAV123 dataset that is
faster than DCF (333 fps) for the OTB100 and KCF (296 fps) for the
...
others. \textit{YoungWoo : For me, it was hard to get the previous sentence.}
To this end, in this paper, we propose a single-target
...
\caption{Examples of three filters with hand-crafted features. The
  \textit{YoungWoo : the responses are target (desired) response for the correlations filters}
...
\cite{zhang2012robust}. The ECO and CCOT trackers \textit{YoungWoo : it should be "use" I think.}originally uses deep
...
combination decreases run-time performance to 317 fps (=416fps - 99fps). 
\textit{YoungWoo : Should it not be 99fps.}
