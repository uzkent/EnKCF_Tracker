...
\cite{hong2015multi}, TLD \cite{kalal2012tracking}, STC \cite{zhang2014fast}, and OAB
...
Table \ref{table:Comparison_to_LCT} presents the performance of
different combinations of the proposed algorithm. In short, we
empirically verified that \textit{E}nKCF is an optimal combination in
terms of tracking accuracy and run-time performance. We could achieve
4.26\% higher in accuracy by running $R_{R}^{t}$ and $R_{S}$ at every
frame, but this combination wills sacrifice run-time performance 317
fps.  To be more specific, we employ $R_{s}$ and $R_{t}^{L}$ or
$R_{t}^{S}$ to run them in every frame in a similar way to the
multiple correlation filter framework of the LCT tracker
\cite{ma2015long}. Running another filter, $R_{t}^{L}$ or $R_{t}^{S}$,
in the {\it E}nKCF framework not only boosts run-time performance but
also increases accuracy. Lastly, we run $R_{L}$, $R_{t}^{S}$ and
$R_{s}$ in every frame.  $R_{t}^{S}$ uses the translation estimated by
...
performance. In summary, running two (complementary) translation
filters and a scale filter as {\it E}nKCF not only increases run-time
performance but also offers more accurate tracker than the baseline
method with one translation and one scale filter.
\begin{table}[!h]
\smaller
\begin{center}
...
\end{tabular}}
\end{center}
\caption{Evaluations on different combinations of the KCFs for UAV123
  dataset. We removed the particle filter from this experiment. The
  $2^{nd}$ method fliped the way of deploying $R_{t}^{L}$ and
  $R_{t}^{S}$ (fig.~\ref{Workflows}). The $3^{th}$ method removed
  $R_{t}^{S}$ and replaces with $R_{t}^{L}$ whereas the $4^{th}$ one
  removes $R_{t}^{L}$. {\it Burak: $3^{th}$ and $3^{th}$?} {\it YoungWoo : I meant to say $4^{th}$} The methods
  marked with "*" run the filters on every frame in the listed
  order. The LCT tracker \cite{ma2015long} runs a translation and
  scale filter in every frame (similar to the methods at the $7^{th}$ and $8^{th}$
  columns) {\it Burak: Not in $7^{th}$?} {\it YoungWoo: I meant to say $7^{th}$ and $8^{th}$} and couples a re-detection
  method to perform long-term tracking at $\approx30$ fps.}
...
the KCFs. In these settings, the {\it E}nKCF with particle filter
achieves $51.98\%$ precision ($20$px), outperforming the baseline {\it
  E}nKCF by $6.32\%$. {\it Burak: the baseline here is the one without
  particle filter?} {\it YoungWoo : Yes, the baseline is the one without Particle Filter.} 
In summary, with the removal of global camera
motion, the integration of particle filter into the {\it E}nKCF as
shown in fig.~\ref{Workflows} further improved the results.
