...
\caption{Evaluations on different combinations of the KCFs for UAV123
  ...
  $R_{t}^{S}$ and replaces with $R_{t}^{L}$ whereas the $3^{th}$ one
  removes $R_{t}^{L}$. {\it Burak: $3^{th}$ and $3^{th}$?} The methods
  marked with "*" run the filters on every frame in the listed
  order. The LCT tracker \cite{ma2015long} runs a translation and
  scale filter in every frame (similar to the method at the $6^{th}$
  column {\it Burak: Not in $7^{th}$?} and couples a re-detection
...
achieves $51.98\%$ precision ($20$px), outperforming the baseline {\it
  ...
  particle filter?} In summary, with the removal of global camera
