...
{\it Burak: What we wrote here didn't match what you told me. Go over
  the following paragraph and change the numbers correctly.}
...
applied to the $i$th image frame, another translation filter,
$R_{t}^{S}$ is applied to the $i+1$th image frame, and then the scale
filter, $R_{s}$ is applied to the $i+2$the image. This order repeats
until the end of the image frames. The translation filter,
$R_{t}^{L}$, is intended to run right after the scale filter, $R_s$,
runs which is applied at every other $i+2$ frames. We run these two
filters this way because we want to minimize any drifts that are
likely to happen running only $R_{s}$. In addition, the filter,
$R_{t}^{S}$, is applied to every other frames before $R_{s}$ and right
after two consecutive frames running $R_{t}^{L}$. By iterating this
order of learning KCFs, we can integrate more discriminative shape
...
and alg.\ref{alg:MKCF} shows the pseudo-code of {\it E}nKCF.

{\it Burak: Remove this EnKCF2 part from the text and from the pseudo code.}

We do not have a separate pseudo-code for {\it E}nKCF2 as there are
only two differences between {\it E}nKCF and {\it E}nKCF2. First, the
{\it E}nKCF2 runs $R_{t}^{L}$ every frame to better tackle large
...
\centering
\begin{tabular}{cc}
\bmvaHangBox{\includegraphics[width=4.0cm]{./figures/Precision_UAV123_10fps.pdf}}
