...
{\it Burak: Rewrite the following paragraph.}

In summary, the filter, $R_{t}^{S}$, will handle the absence of scale
...
$R_{S}$ and $R_{t}^{S}$ addresses the weakness of $R_{t}^{L}$ that it
can not learn very useful shape features
alone. 

Figure \ref{Workflow_figure_EnKCF} illustrates the workflow of the
proposed algorithm and Figure \ref{alg:MKCF} shows the pseudo-code of
{\it E}nKCF.
...
The proposed {\it E}nKCF algorithm can cause undesired drift and noise
in tracking due to independent and less frequent application of the
correlation filters. 

{\it Burak: When we explain EnKCF, we've already used potential drifts
  are the reason why we want to have three KCFs. If we're talking
  about the same, kind of drifts, you have to be more careful. Having
  this section about particle filter will make it impractical or
  usefulness to have three KCFs.}

To mitigate the drift effect further and reduce the noise, a Bayes
Filter representing the evolution of target's motion can be added into
our {\it E}nKCF tracking framework. To achieve this, we consider a
variant of a Bayes Filter, \textit{Particle Filter}. The Particle
...
$N$ denotes the size of the window in consideration. We can also use
the spatial distance of particles to the peak of the confidence map,
however, we believe that this approach could result in further drifts
...
approximated posterior pdf is computed as
\begin{equation}
	\hat{X}_{t} = \sum_{p=1}^{P}w_{p_{t}} X_{t}.
\end{equation}
The particles are assigned equal weight after the importance
...
area. In this case, the particles outside of the scale filter ROI do
not have correspondence in the confidence map. Additionally, it is
...
$4$th in AUC. However, it is $20$-$30$ faster than the ones with
higher AUC.

\begin{figure}
\centering
\begin{tabular}{cc}
\bmvaHangBox{\includegraphics[width=4.00cm]{./figures/Precision_UAV123_10fps.pdf}}
\bmvaHangBox{\includegraphics[width=4.0cm]{./figures/Success_UAV123_10fps.pdf}}\\
\end{tabular}
