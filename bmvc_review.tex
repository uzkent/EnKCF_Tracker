...
$R_{t}^{S}$, and $R_{s}$ differently as $0.012$, $0.020$ and
...
ratio. Finally, the padding size for the correlation filters is tuned to $3$ for $R_{t}^{L}$, $2$ for $R_{t}^{S}$, 
and $1$ for $R_{s}$. For our particle filter implementation, we empirically set the
number of particles to $1000$ to balance the run-time performance and
accuracy. To keep the level of variance among the particles
...
and color-naming \cite{van2009learning} features to learn the
background around the target more effectively. However we found,
through experiments, that only fHoG didn't work well to cover a larger
ROI. In a same way, we found that another translation filter,
$R_{t}^{S}$, worked well when it used fHoG features only and the scale
filter, $R_{s}$ did when it used both fHoG and color features.

\textbf{Performance on UAV123 Dataset} We compared, using the
precision and success rate metrics, the performance of the proposed
...
lower-speed ($\geq$50): SAMF\cite{li2014scale},
...
high-speed tracking methods by $3\%$-$15\%$ in 20 pixels
...
\raggedright
\begin{tabular}{ccc}
\tiny\:\:\:\:\:\:\textbf{UAV123}\hspace{.44\linewidth}\:\:\:\textbf{OTB100}\\
\bmvaHangBox{\includegraphics[width=3.00cm]{./figures/pr_uav123.eps}}
\bmvaHangBox{\includegraphics[width=3.20cm]{./figures/sr_uav123.eps}}
\bmvaHangBox{\includegraphics[width=3.00cm]{./figures/pr_otb100.eps}}
\bmvaHangBox{\includegraphics[width=3.20cm]{./figures/sr_otb100.eps}}\\
\end{tabular}
...
We also evaluate the performances of different combinations of the proposed correlation filters and 
compare them to {\it E}nKCF as shown in table~\ref{table:Comparison_to_LCT}.  
By using three KCFs ( $R_{t}^{L}$, $R_{t}^{S}$, $R_{s}$) in {\it E}nKCF,
we can achieve higher accuracy than using $R_{s}$ and $R_{t}^{L}$ or
$R_{t}^{S}$ with similar run-time performance. On the other hand, we
employ $R_{s}$ and $R_{t}^{L}$ or $R_{t}^{S}$ and run them in every frame in a similar way
to the multiple correlation filter framework of the LCT tracker \cite{ma2015long}. Running another filter, $R_{t}^{L}$ or
$R_{t}^{S}$, in the {\it E}nKCF framework not only boosts run-time performance but also increases
accuracy. Finally, we run  $R_{L}$, $R_{t}^{S}$ and $R_{s}$ in every frame in the in the given order. 
$R_{t}^{S}$ uses the translation estimation given by
$R_{t}^{L}$ to update translation and finally $R_{s}$ uses the final
translation estimation given by $R_{t}^{S}$ to update the
scale. Interestingly, running two complementary translation filter
results in significant improvement at the cost of decreased run-time
performance. In conclusion, running two complementary translation
filter and a scale filter in the {\it E}nKCF framework not only
provides high speed but also more accurate tracker than the baseline
methods having one translation and one scale filter.
\begin{table}[!h]
\smaller
\begin{center}
...
\end{tabular}}
\end{center}
\caption{Evaluations of the different combinations of the proposed
  KCFs in UAV123 dataset. The particle filter is decoupled from all the methods for fair comparison. 
  The $2^{nd}$ method swaps the order of
  $R_{t}^{L}$ and $R_{t}^{S}$ (fig.~\ref{Workflows}). The $3^{th}$
  method removes $R_{t}^{S}$ and replaces with $R_{t}^{L}$ whereas the
  $3^{th}$ one removes $R_{t}^{L}$. The methods denoted with "*" run
  the filters in every frame in the listed order. The LCT tracker
  \cite{ma2015long} runs a translation and scale filter in every frame
  (similar to $6^{th}$ method) and couples a re-detection method to
  perform long-term tracking at $\approx30$ fps.}
\label{table:Comparison_to_LCT}
\end{table}

Finally, on the $UAV123$ dataset, we compare the {\it E}nKCF with particle filter to the one without particle filter. To do so, we use 50 sequences labeled as no abrupt camera motion in the dataset. To evaluate the robustness of particle filter, we add noise, drawn from a uniform distribution ($[-20,20]$), to the translation estimations of the KCFs. In these settings, the {\it E}nKCF with particle filter achieves $51.98\%$ precision ($20$px), outperforming the baseline {\it E}nKCF by $6.32\%$.  In conclusion, with the removal of global camera motion, the integration of particle filter into the {\it E}nKCF as shown in fig.~\ref{Workflows} can further improve the results.
...
\bmvaHangBox{\includegraphics[width=3.20cm]{./figures/sr_uav123_10fps.eps}}\\
\end{tabular}
