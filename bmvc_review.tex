...
We also evaluate the performances of the different combinations of the
proposed correlation filters and compare them to the {\it E}nKCF as shown in table~\ref{table:Comparison_to_LCT}.  
...
\caption{Evaluations of the different combinations of the proposed
  KCFs in UAV123 dataset. The $2^{nd}$ method swaps the order of
...
Finally, on the $UAV123$ dataset, we compare the {\it E}nKCF with particle filter and without particle filter. To do so, we use the 50 video sequences with no abrupt camera motion as annotated by the authors. Also, we add noise, drawn from a uniform distribution ($[-20,20]$), to the translation estimations of the KCFs to further evaluate the robustness of particle filter. In this settings, the {\it E}nKCF with particle filter achieves $51.98\%$ precision ($20$px), outperforming the baseline {\it E}nKCF by $6.32\%$.  In conclusion, the proposed particle filter can be a useful tool in {\it E}nKCF framework with the removal of global camera motion.
