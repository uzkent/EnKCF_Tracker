...
offline training. Specifically, we would like to make our algorithm 1)
learn the appearance model of a target on the fly and 2) run as fast
on a typical desktop as up to $300-400$ fps so that it could run, at
least, faster than 30 fps when it is deployed on a low-end, embedded
...
and translation, but in a more efficient way. In particular, we use an
ensemble of KCFs in turn, instead of running them all together on
...
($R_{t}^{L}$). By doing so, we could address the scale change and
estimate target's motion efficiently while maintaining or increasing
