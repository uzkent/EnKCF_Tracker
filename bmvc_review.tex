...
\begin{comment}\item \textbf{Re-Detection} A target loss is inevitable in object
...
In this study, we propose application of an ensemble of KCFs
...
scale filter at each frame decreases run-time performance ($\leq50 fps$)
...
aspects of tracking and addressing the weakness of each other,
resulting in more robust tracking. We name these filters as
\textit{target}+\textit{small background} translation filter
($R_{t}^{S}$), \textit{target-only} scale filter ($R_{s}$) and
\textit{target}+\textit{large background} translation filter
($R_{t}^{L}$). The fig.~\ref{fig:Filters} shows the size of the ROIs associated with
$R_{t}^{S}$, $R_{t}^{L}$, and $R_{s}$ in addition to the hanning
windows assigned and desired Gaussian responses assigned to each one. Unlike \cite{ma2015long}, by running a single filter at
each frame, we can achieve the targeted operation frame rate. In our framework, each filter is only trained after it is run to estimate translation or scale. This makes more sense as neighboring frames show large correlation. Thus, skipping a few frames to train a filter does not result in dramatic information loss in the learned object model. 
...
estimation module and earning us some time to update scale. Finally, the scale filter help us on the track
re-initialization module which includes running an object proposals
algorithm to extract proposals from salient regions. By using the
...
$R_{t}^{L}$. It estimates only translation similar
to \cite{henriques2015high}. Finally, the \textit{target+large
  background} area translation filter, $R_{t}^{L}$ , estimates translation of a target as $R_{t}^{S}$. We propose scale estimation at every $n$ number of frames using the scale filter. In our experiments on $OTB100$ and $UAV123$,
...
in the next frame after the scale filter as we want to avoid drifts which is more likely to happen running only $R_{s}$. Considering a large ROI for $R_{t}^{L}$ can degrade its performance due to information loss resulting from ROI resizing. On the other hand, only HoG features are used in $R_{t}^{S}$ as it focuses on a smaller ROI, giving us more discriminative HoG features. In our framework, $R_{t}^{S}$ addresses the absence of scale adaptation module in $R_{t}^{L}$ and $R_{t}^{S}$ whereas $R_{t}^{L}$ focuses on addressing the absence of translation estimation module in $R_{S}$. $R_{t}^{S}$, on the other hand, focuses on the shape features of the target that is not fully exploited in $R_{t}^{L}$ and $R_{t}^{S}$. For this reason, we employ \textit{HoG} features in $R_{t}^{S}$ and \textit{HoG} and \textit{Color} features in $R_{t}^{L}$. Fig.~\ref{Workflow_figure_EnKCF} displays the proposed tracking framework called Ensemble of KCFs {\it E}nKC. Also, the pseudo-code of the proposed {\it E}nKC is shown in alg.~\ref{alg:MKCF}.
...
cases such as VOT15 dataset. Its computational efficiency arises from its use of the
