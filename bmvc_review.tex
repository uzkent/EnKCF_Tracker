...
is deployed on a low-end, embedded system.\footnote{We empirically
  come up with this computation about an 1/10th scale down of
  computational resources from a typical PC to an embedded system.}
...
\caption{Examples of tracking results by the proposed method on the
  ``UAV123'' dataset. This ``UAV123'' dataset is challenging for
  object tracking as a target's scale is changed drastically with a
  few frames.}
...
ROIs associated with $R_{t}^{S}$, $R_{t}^{L}$, and $R_{s}$.
