...
\end{tabular}
\caption{The work-flow of {\it E}nKCF tracking algorithm.}
\label{Workflows}
\end{figure}
...
\Begin($\text{track($x_{t-1},y_{t-1},s_{t-1}$) }$)
{
	...
	Crop the ROI for the $R_{t}^{L}$ (Condition 2), or $R_{t}^{S}$ (Condition 3) given $(x_{t},y_{t})$ and estimate new position ($x_{t}$,$y_{t}$),\\
	// \textit{Scale Estimation - Correlation Filter}\\
	Crop the ROI for the $R_{s}$ and estimate scale, $s_{t}$ (Condition 1),\\ 
	Scale pool for $R_{s}$ : $\lbrace1.05,1.0,1/1.05\rbrace$,\\
	Skip it for $R_{t}^{L}$ and $R_{t}^{S}$ ($s_{t}$=$s_{t-1}$)\\
...
\caption{{\it E}nKCF Tracking Algorithm}\label{costalgorithm}
\end{algorithm}
...
%				and estimate translation ($x_{t}$,$y_{t}$),
...
%		        \State Skip it for $R_{t}^{L}$ and $R_{t}^{S}$ ($s_{t}$=$s_{t-1}$),
...
\caption{Examples of three filters; the translation filter,
  ...
  expected Gaussian response. Instead of using a single, correlation
  filter for estimating change of a target's scale and translation, it
  would be better to use more than one correlation filter for better
  estimating change of scale and translation individually.}
\label{fig:Filters}
\end{figure}
...
tracking method and the theory and its work flow has been detailed in
other papers already \cite{henriques2012exploiting,henriques2015high}. This section briefly
goes over the parts of the KCF relevant to this study. Its
computational efficiency is derived from the correlation
filter framework representing training examples as a circulant matrix. To reduce complexity, the
circulant matrix can be diagonalized by Discrete Fourier Transform.  This way, off-diagonal elements become
zero whereas the diagonal elements represent the eigenvalues of the circulant matrix. The egienvalues are equal
to the DFT transformation of the base sample ($x$) elements. The Kernelized Correlation Filter, on the other hand,
applies a kernel to $x$ to transform to a more discriminative domain. The circulant matrix is then formed by applying
cylic shifts on the kernelized $x$. Such kernelization operation does not introduce additional complexity to the baseline
...
runs on an embedded system in ``high speed,'' $\ge 300$ fps, e.g.,
running our tracking algorithms on a drone. Given this, we believe
that the UAV123 video data is a better fit in evaluating the
performance of the proposed algorithm than any other existing tracking
datasets. This is because there are many challenging scenes, e.g.,
...
applications, it is important to compare the performance of the
proposed algorithm with those of the state-of-the-art in nominal
...
perspective view. Finally, the {\it E}nKCF2 is tested on temporally
down-sampled version of the UAV123 dataset, called UAV123$\_$10fps dataset. 
This way, by modifying the proposed {\it E}nKCF framework slightly we can tackle scenarios dominated by extreme camera and object
motion as we run multiple correlation filter at each frame. One can also perform such modifications based on the hardware specifications. 
A more sophisticated hardware can be more friendly to apply multiple correlation filter ({\it E}nKCF2) at one frame
whereas the ones on the low end can only accommodate a single correlation filter at each frame ({\it E}nKCF).

{\it Burak: You need to explain why it is useful or meaningful to
  evaluate our algorithm w/ a down-sampled version of the UAV123
  dataset.}

{\it YoungWoo: I edited the preceding paragraph based on your comment.}
...
{\it Burak: I don't think I agreed to put the following part as a part
  of our submission.}
...
\end{tabular}
\caption{Evaluation and comparison of the proposed {\it E}nKCF2 tracker on the UAV123$\_$ dataset in terms of the precision and success rates. This dataset consists of temporally down-sampled $123$ video sequences captured from a micro UAV.}
