...
\hline
Method & {\it E}nKCF & $R_{t}^{S}$+$R_{t}^{L}$+$R_{s}$ & $R_{t}^{L}$+$R_{s}$ & $R_{t}^{S}$+$R_{s}$ & $R_{t}^{L}$+$R_{s}$* & $R_{t}^{S}$+$R_{s}$* & $R_{t}^{L}+R_{t}^{S}$+$R_{s}$* \\
\hline\hline
Pr. ($20$px) & \color{blue}53.9 & 48.93 &52.41 & 48.10 & \color{red} 55.85 & 52.14 & \color{green}58.16 \\
SR ($50\%$) & \color{red} 40.2 & 36.75 & 38.23 & 36.04 & \color{blue}39.89 & 38.51 & \color{green}41.58 \\
FPS & \color{red} 416 & \color{blue}412 & 370 & \color{green}425  & 135 & 151 & 99 \\
\hline
\end{tabular}}
\end{center}
\caption{Evaluations of the different combinations of the proposed KCFs in UAV123 dataset. The $2^{nd}$ method swaps the order of $R_{t}^{L}$ and $R_{t}^{S}$ (fig.~\ref{Workflows}). The $3^{th}$ method removes $R_{t}^{S}$ and replaces with $R_{t}^{L}$ whereas the $3^{th}$ one removes $R_{t}^{L}$. The methods denoted with "*" run the filters in every frame in the listed order. The LCT tracker \cite{ma2015long} runs a translation and scale filter in every frame (similar to $6^{th}$ method) and couples a re-detection method to perform long-term tracking at $\approx30$ fps.}
