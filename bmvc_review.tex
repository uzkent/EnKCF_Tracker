...
tracker and run $R_{t}^{L}$, $R_{t}^{s}$, and $R_{s}$ in every frame in the given order. Interestingly, running two complementary translation filter results in significant improvement
...
\hline
Method & {\it E}nKCF & $R_{t}^{L}$+$R_{s}$ & $R_{t}^{S}$+$R_{s}$ & LCT  ($R_{t}^{L}$+$R_{s}$) & LCT  ($R_{t}^{S}$+$R_{s}$) & LCT  ($R_{t}^{L}+R_{t}^{S}$+$R_{s}$) \\
\hline\hline
Pr. ($20$px) & \color{red} 53.9 & 48.09 & 52.50 & & 52.14 & \textbf{58.16} \\
SR ($50\%$) & \color{red} 40.2 & 36.04 & 37.85 & & 38.51 & \textbf{41.58} \\
FPS & \color{red} 416 & 370 & \textbf{425}  & & 151 & 99 \\
\hline
\end{tabular}
\end{center}
...
\centering
\begin{tabular}{cc}
\bmvaHangBox{\includegraphics[width=4.30cm]{./figures/Precision_UAV123_10fps.eps}}
\bmvaHangBox{\includegraphics[width=4.30cm]{./figures/Success_UAV123_10fps.eps}}\\
\end{tabular}
