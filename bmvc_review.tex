...
cases \cite{henriques2012exploiting,henriques2015high}. Its computational efficiency arises from its use of the
discrete fourier transform of the circulant matrix and frequency
domain. The KCF tracker minimizes the regularized ridge regression function shown below.
\begin{equation}
...
derivative of $E$ w.r.t $w$ to $0$. 
\begin{comment}The primal solution in the
...
%\end{equation}
\end{comment}
Then, the Circulant-matrix based solution vector in the Fourier domain $\hat{w}$ is formulated as
\begin{equation}
...
$0.010$. For the kernel of the Gaussian neighboring function, we empirically
...
The re-sampling is only performed when the \textit{efficient number of samples} ($	\hat{N}_{eff} \approx (\sum_{p=1}^{P}w_{p}^{2})^{-1}
$) is lower than a pre-defined threshold. This way, we can continuously keep reasonable variance among particles.
\begin{equation}
	\label{eq:Neff_PF}
\end{equation}
...
\textit{Success\:Rate} metrics. In precision, we rank the trackers based on the precision numbers at
20 pixels whereas in the success rate plots, trackers are
ranked based on area under curve (AUC) scores.
...
\cite{henriques2012exploiting}, DCF\cite{henriques2015high}, and MOSSE\cite{bolme2010visual,henriques2015high}. Also, five relatively
...
DSST\cite{danelljan2014accurate}, Struck\cite{hare2012efficient}, MUSTER \cite{hong2015multi}. TLD \cite{kalal2012tracking}, and
OAB \cite{zhang2012robust}. Fig.~\ref{fig:UAV123_OTB100_DATASET} shows the results on the UAV123
dataset. 
...
second best tracker in terms of \textit{area under curve} (AUC) value
...
$5\%$ behind of another low-speed scale adaptive SAMF tracker as shown in fig.~\ref{fig:UAV123_OTB100_DATASET}. As in
UAV123 dataset, the {\it E}nKCF performs exceptionally in handling the
scale changes. It is ranked as the third best tracker while being the
second highest speed tracker behind MOSSE. These numbers
