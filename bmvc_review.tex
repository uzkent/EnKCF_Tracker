...
of the {\it E}nKCF tracker on the OTB100 dataset, and observed a
performance similar to that of UAV123 data. Figure
\ref{fig:UAV123_OTB100_DATASET} shows the results on the OTB100
dataset. For the precision metric, {\it E}nKCF performed slightly
...
As in UAV123 dataset, the {\it E}nKCF
performs exceptionally in handling the scale changes. It is ranked as
the third best tracker while being the second highest speed tracker
behind MOSSE. These numbers prove that {\it E}nKCF is not only a good
candidate for tracking on a UAV-based mobile platform but also mobile
platforms on smart phones.
\begin{figure}[!h]
\centering
\begin{tabular}{ccc}
