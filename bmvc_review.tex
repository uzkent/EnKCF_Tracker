...
algorithm running a very high-speed, $\geq 300$ fps. In particular,
...
large camera motion. To do so, we slightly modify {\it E}nKCF by running $R_{t}^{L}$ every frame and remove the particle
filter due to large ego-motion. 

In particular, the UAV123 data has another version where the original
dataset was temporally down-sampled at $10$ fps and makes the
challenging original dataset, because of objects in fast motion and
large ego-motion, more challenging. This is because the magnitude of
those motions is even bigger. Figure \ref{fig:UAV123_10fpsDATASET}
