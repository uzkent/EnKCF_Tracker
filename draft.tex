...
addition, we devised a Bayes filter for a smooth transition on
execution of individual KCFs. To minimize the effect of drifting, we
developed a re-detection module that detects target-lost, uses an
...
69.53\% for precision on 20 pixels and 52.90\% for success rate for
OTB100 data, and 54.11\% and 40.21\% for UAV123 data. These results
confirmed that our method outperformed the existing ones over 5\% on
precision on 20 pixels and 3\% on AUC on average. Our implementation
ran at 340 fps for OTB100 and at 416 fps for UAV123 data that is
faster than DCF (292 fps) for OTB100.
\end{abstract}
