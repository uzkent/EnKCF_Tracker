...
%%%%%%%%% TITLE
\title{{\itE}nKCF: Ensemble of Kernelized Correlation Filters for Object Tracking in High Speed, 300Hz}
...
\begin{comment}
Maybe the goal of this work is to public a journal paper about a
practical solution for a drone to follow a single target. For this, we need
\begin{itemize}
...
part. This is because we need to build a drone for the second part.
\end{comment}
...
\item Re-detection for re-initialization
\end{itemize}
\item Experiment: Detail the data, and experimental setup; results and findings
\item {\it E}nKCF, Naive{\it E}nKCF, ${\it E}nKCF_{+RD}$, LCT, DSST,
  vanilla-KCF w/ a fixed scale, KCF w/ three different scales, STC,
\begin{itemize}
\item DTB-70: w/ and w/o motion estimation,
\item UAV data, https://ivul.kaust.edu.sa/Pages/Dataset-UAV123.aspx
\item VOT
\end{itemize}
\item Conclusion
\end{itemize}
...
target. The recent examples of the tracking-by-detection algorithms
include Tracking-Learning-Detection \cite{kalal2012tracking}, Multiple
