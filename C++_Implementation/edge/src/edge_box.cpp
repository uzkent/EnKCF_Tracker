...
void boxesNms(Boxes &boxes, float thr, float eta, int maxBoxes)
{
  ...
  while (i<n && m<maxBoxes) {
    ...
    if (keep) {
      kept[b].push_back(boxes[i]); m++;
    }
    i++;
    if (keep && eta<1 && thr>.5) {
      thr *= eta; d = ceil(log(1 / thr) / lstep);
    }
  }
  boxes.resize(m); i = 0;
  for (j = 0; j<nBin; j++)
...
void EdgeBoxes(float* _E, float* _O, int h, int w, edge_box_parameters opt, float* bbs, int* num)
{
  arrayf E; E._x = _E;
  arrayf O; O._x = _O;
  O._h = E._h = h;
  ...
  edgeBoxGen._alpha = opt.alpha;
  edgeBoxGen._beta = opt.beta;
  edgeBoxGen._eta = opt.eta;
  edgeBoxGen._minScore = opt.minScore;
  edgeBoxGen._maxBoxes = opt.maxBoxes;
  edgeBoxGen._edgeMinMag = opt.edgeMinMag;
  edgeBoxGen._edgeMergeThr = opt.edgeMergeThr;
  edgeBoxGen._clusterMinMag = opt.clusterMinMag;
  edgeBoxGen._maxAspectRatio = opt.maxAspectRatio;
  edgeBoxGen._minBoxArea = opt.minBoxArea;
  edgeBoxGen._gamma = opt.gamma;
  edgeBoxGen._kappa = opt.kappa;
  ...
  *num = (int)boxes.size();
  for (int i = 0; i<*num; i++) {
    bbs[i * 5] = (float)boxes[i].c;
    bbs[i * 5 + 1] = (float)boxes[i].r;
    bbs[i * 5 + 2] = (float)boxes[i].w;
    bbs[i * 5 + 3] = (float)boxes[i].h;
    bbs[i * 5 + 4] = boxes[i].s;
  }
}
