...
background bigger than that of the first filter, $R_{t}^{S}$, \textit{target}+\textit{large background}. 
...
Figure \ref{fig:Filters} shows examples of three filters used in  {\it E}nKCF.
...
will detail the {\it E}nKC with hand-crafted features, dedicated to run
on low-end embedded systems, and deep features, designed to run on
the systems with GPUs. Later, we will briefly document the fundamental 
concepts of the KCF and Particle Filter, designed to smooth transitions 
between KCFs.
...
placed them after the Kernelized Correlation Filter section. To me, it makes more
sense to place them right after the EnKCF section.}
...
We further improved the performance by employing deeper features in the scale filter 
with zero padding around the target.
