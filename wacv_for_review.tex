...
We, first, use two
conventional features, fHoG \cite{felzenszwalb2010object} and
color-naming \cite{li2014scale}, to make {\it E}nKCF run on any
embedded systems faster than 30 fps. {\it Burak: You need to make
  these two sentences connected -- the one begin with ``We, first,
  use,'' and ``Figure ..''}

Figure \ref{fig:Filters} shows examples of the region of interest
(ROI) associated with $R_{t}^{S}$, $R_{t}^{L}$, and $R_{s}$. 

We used different features for each of three KCFs to address different
challenges. In particular, the translation filter, $R_{t}^{L}$, uses
...
around the target as shown in Figure~\ref{fig:Filters}. {\it Burak:
  There are three subfigures in Figure \ref{fig:Filters}. You need to
  tell the readers of this paper which one they need to look at if
  they want to.}

The color features compensate for the noisier shape features due to
the large ROI considered by $R_{t}^{L}$. {\it Burak: You meant ``the
  color features are used to compensate for shape features' weakness
  -- what's weakness?}

On the other hand, color features is not assigned to $R_{t}^{S}$ as it
learns more useful shape features than $R_{t}^{L}$ due to smaller
padding size. {\it Burak: color features are not used for $R_{t}^{S}$ because?}

Both fHoG and color-naming features are used for $R_{s}$ to ? {\it
  Burak: DO NOT USE $R_s$ just only, alway use them with their names,
  the scale filter. This would just confuse the reader! Don't expect
  for them to remember what you said earlier!}

since we believe that the lower template resolution (64 x n)
assigned to $R_{s}$ might lead to less discriminative shape
features. 

Smaller template size is assigned to $R_{s}$ as it estimates
the scale by correlating the filter with three ROIs as shown in
...
fluctuating run-time performance which is not desired in embedded
systems.
...
as features for this idea where their experiments showed that the
...
third convolutional layer features to estimate the response map. In
...
stage. This coarse-to-fine position estimation approach accommodates
...
$R_{s}$. Taking this into account, we can assign deeper feature
...
%---------------------------------------------------------------------- 
{\it Burak: We cannot just begin with a detail. You need to give the
  readers an overview of what they'll be reading.}
...
{\it Burak: No summary on EnKCF w/ deep features?}
