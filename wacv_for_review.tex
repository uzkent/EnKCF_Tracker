...
not require offline training and can run in a
high-speed. Specifically, we would like to have our algorithm 1) learn
the appearance model of a target on the fly and 2) run as fast on a
typical desktop as up to $300$-$450$ fps.
...
KCFs, we developed a particle filter. In addition, to increase the
...
perform a single-target tracking while effectively addressing
...
whereas in the success rate plots, they are ranked based on area under
...
convolutional features, however, for the fair comparison, we employ
...
\ref{fig:UAV123_DATASET} shows the results on the UAV123 dataset. The
    ...
    more than $10$ and $30$ times faster. It is somewhat obvious that
    why the {\it E}nKCF outperformed the DSST because our scale filter
    learned more information about the target because ours used a 2-D
    filter, instead of a 1-D one and both fHoG and color-naming
    features, instead of just fHoG.
...
a tracking algorithm. To this end, we temporarily down-sampled the
UAV123 called UAV123$\_$10fps dataset. We believe the downsampling
would make the original UAV123 data more challenging because the
displacements of the moving objects get even bigger. We also slightly
...
tracking methods. The precision rates of the ECO and CCOT's dropped
about $10\%$ from those with the original UAV123 dataset. Our tracker
...
interesting to observe how sensitive the frame rate is to the
...
training and execute in high-speed. To develop a single-target
...
algorithm improve the performance by $5\%$.
