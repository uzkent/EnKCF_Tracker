...
\end{equation}
where $\lambda$ represents the regularization weight whereas $\hat{k}^{xx}$ denotes
 the first row of the kernel matrix $K$ known as \textit{gram matrix} and it is expressed as
\begin{equation}
...
computed as, $w_{p_{t}}(x_{t},y_{t}) = \sum_{i=1}^{N}\sum_{j=1}^{M} y_{R}(x_{t}-i,y_{t}-j)$,
