...
\end{equation}
A non-linear version of $\boldsymbol{\hat{w}}$, $\boldsymbol{\hat{\alpha}}$ , is proposed to increase robustness
...
coarse-to-fine translation estimation approach accommodates different levels of abstractions
...
$\boldsymbol{X_{t}} = \lbrace x, y, v_{x}, v_{y} \rbrace$, where $x$ and
...
\boldsymbol{y_{R}}(x_{t}-i,y_{t}-j)$, where $w_{p}$ denotes the weight of
...
dataset. The runtime performance the Deep{\it E}nKCF (conv224) is 30.74 fps 
whereas the conv222 setting yields to a 35.23 fps tracker on a CPU. }
