...
area is tended to be noisy and less discriminative and adding color
...
target, the reason of using two feature sets is sightly different in
...
in alg.~\ref{alg:MKCF}.  This increases the complexity from
...
\textit{conv224-VGG}. {\it Burak: I don't see any differenciation
  between conv222-VGG and conv224-VGG from the figure, at least labels
  of them.} We use the VGGNet since it provides higher spatial
...
\caption{The proposed conv224-VGGNet feature extraction in Deep{\it E}nKCF. $R_{t}^{L}$ and $R_{t}^{s}$ used an hanning window to avoid distortion caused by FFT operation whereas $R_{s}$ is not filtered {\it Burak: What do you mean by ``not filtered''?} to avoid target boundary information loss.}
...
the feature vectors learned from DNN have been used to improve the
...
could be also improved with deep features trained on ImageNet dataset
and like to see how much performance boost we could gain. 
\begin{comment}
...
results that we ran Deep{\it E }nKCF on UAV123 Dataset. Two Deep {\it
  E}nKCF settings outperformed those with the HoG and color features
...
works better for the smaller ROIs. {\it Burak: You'll have to explain 1) why, you think, DeepEnKCF worked better, 2) why DeepEnKCF didn't perform a way better than EnKCF, 3) why did conv224-VGG perform better than conv222-VGG.}
\begin{figure}[!h]
\centering
\begin{tabular}{ccc}
...
\end{tabular}
\caption{{\it Burak: Make this figure bigger if the space permitted.} Comparison of {\it E}nKCF's ({\it fhog} + {\it color-naming}) performance with Deep{\it E}nKCF on the UAV123
dataset.}
...
$5\%$ on embedded systems with GPUs. 
