...
originally uses deep convolutional features. For the sake of fair comparison, we employ fHoG
and color-naming features similar to our study. Figure \ref{fig:UAV123_DATASET} shows 
the results on the UAV123 dataset. {\it  E}nKCF outperformed most of the high-speed tracking methods by
...
of those motions is even bigger. Also, we can observe the sensitivity of 
the state-of-the-art lower speed trackers to the video frame rate b
y running them on this downsampled dataset. Figure \ref{fig:UAV123_10fpsDATASET}
...
In the lower frame rate scenarios (10 fps), the ECO and CCOT's precision rates drop about
$10\%$ worse than the original frame rate scenarios. We believe that this is an indicator of
how good of a performance we can expect from these trackers on low-end hardware. The projected
