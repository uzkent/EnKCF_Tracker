...
In particular, the translation filter, $R_{t}^{L}$, employs
...
compensate for the noisier fHoG features due to the large background area considered 
...
than  $R_{t}^{L}$}. Finally, both fHoG and color-naming features are used in the scale filter, $R_{s}$, 
since the ROI is resized to a lower template resolution (64 x n) than the translation filters. This leads to 
less discriminative shape features as in Figure~\ref{fig:Filters}.c. The $R_{s}$ estimates
...
correlation filter on a single ROI. By doing so, fluctuating run-time performances among KCFs are avoided which is
desired in embedded systems.
...
To further improve the performance, we employed deeper features in the scale filter since it has zero
