...
and KCF (292 fps), DCF (457 fps) for UAV123. Finally, for the embedded
systems with GPUs, we explored different levels of deep features for different
correlation filters to boost tracking performance. 
\end{abstract}
...
$R_{t}^{L}$, and $R_{s}$ assigned hand-crafted features, . This version of {\it E}nKCF is dedicated to run
on any embedded system at real-time ($30$fps). 


\textbf{{\it E}nKCF with Convolutional Neural Network Features}
In addition to hand-crafted featurres, we explore deep features
to boost tracking performance. This version of {\it E}nKCF, on the other hand, 
is dedicated to run on high-end embedded systems (GPUs). 

There have been many studies on applying a pretrained CNN features to
...
to get the response map. In the next stage, they used the estimation
of the target's transition from the first stage to use the second
convolutional layer features. Finally, the first convolutional layer
features is extracted from the position estimated in the second
stage. This coarse-to-fine position estimation approach allows to
utilize more representative features of the object. However, such
approach requires multiple runs of a KCF at a single frame, increasing
the run-time complexity. For this reason, we follow a similar approach
to \cite{danelljan2015convolutional} to embed deep learning features.

Our {\it E}nKCF framework provides us with a flexibility to exploit different
level of abstractions in different KCFs. The translation filters,
$R_{t}^{L}$ and $R_{t}^{S}$, considers more than twice bigger area
...
%---------------------------------------------------------------------- 
To evaluate the performance of the proposed  {\it
  E}nKCF with hand-crafted features, we choose two
...
KCFs based trackers \cite{ma2015hierarchical,
  danelljan2015convolutional}. We hypothesize that the {\it E}nKCF
tracker could perform better with deep features trained on ImageNet
dataset. Utilizing pretrained deep features would not be applicable to
all the a single-board, embedded system due to their limited
computational resources. But as these days some of the single-board
systems offer GPU, we would like to see how much accuracy we can
achieve by feeding {\it E}nKCF w/ conv features as shown in figure~\ref{fig:Filters_CNN}.
\begin{comment}
