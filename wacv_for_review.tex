...
hand, a translation filter, $R_{t}^{L}$, is designed to look at a
...
learning rates of individual filters, $R_{t}^{L}$,
...
\caption{Results of running different combinations of the KCFs for
  ...
  combining multiple KCFs as long as the deployment order is optimal. The '$*$' represents
...
at every frame, but this combination decreases run-time
performance to 317 fps. Lastly, we run $R_{L}$, $R_{t}^{S}$
...
DeepE{\it n}KCF's performance on UAV123 Dataset. Deep{\it
