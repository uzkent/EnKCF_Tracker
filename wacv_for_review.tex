...
OTB100 and KCF (292 fps), DCF (457 fps) for UAV123. In addition, to
increase flexibility of the proposed {\it E}nKCF running on various
one-chip computer, e.g., one with GPUs, we explored different levels
of deep features.
\end{abstract}
...
and dynamic maneuvers of a target. To maintain or even improve the
run-time performance of the original KCF (e.g., $\ge$ 300), we deploy
three KCFs in turn, instead of applying them on the same image frame
...
right scale of the target. {\it Burak: We only explained two kinds of
...
{\it Burak: DO NOT use ``Figure~\ref{fig:Filters}'' just write ``Figure \ref{fig:Filters}''}
\begin{comment}
 as shown in Figure~\ref{fig:Filters}.
\end{comment}

Figure \ref{fig:Filters} shows examples of three filters.
...
\caption{Examples of three filters with hand-crafted features: the
  translation filter, \textit{target+small background}, another
  translation filter for capturing the bigger background,
  \textit{target+large background}, and the scale filter,
  \textit{target-only} depicted in their Hanning windows and expected
  Gaussian response.}
\label{fig:Filters}
\end{figure}
...
learning only $R_{t}^{L}$. This is because it incorporates a large
area about background in the learned model. The filter, $R_{t}^{L}$,
uses shape and color information together to recover from any
potential drifts -- drifts potentially occurring only by learning the
target's scale with the scale filter, $R_{s}$.
...
designed to addresses the drawback of $R_{t}^{L}$ that may learn noisy
shape features due to its large search area. In what follows, we will
detail each part of the {\it E}nKCF.
...
{\it Burak: This section does NOT connect to the previous section.}

We, first, use two hand-crafted features, fHoG
\cite{felzenszwalb2010object} and color-naming \cite{li2014scale} in
the {\it E}nKCF as shown in Figure~\ref{fig:Filters}. {\it Burak:
  Figure \ref{fig:Filters} is placed a way before this sentence -- We
  don't want the readers to look back while reading a certain
  sentence. And I don't see fHoG and color-naming or descriptions of
  them from that Figure.} 

This version of the {\it E}nKCF is designed to execute a single,
target tracking on any embedded systems at $30$ fps.
...
feature, the bigger the area for HoG is the more noises the resulting
...
translation filter, $R_{t}^{S}$, for capturing a smaller area. This is
because the translation filter, {\it target + small background}, does
not contain much of the background area around the target. Figure
\ref{fig:Filters} (a) shows an example of this translation filter.

\begin{comment}
On the other hand, the color features is not assigned to the small
area translation filter, $R_{t}^{S}$, as it learns more useful fHoG
features than $R_{t}^{L}$ due to smaller padding size as seen in
Figure~\ref{fig:Filters}.a. {\it Burak: color features are not used
  for $R_{t}^{S}$ because?  \it YoungWoo : We do not use color
  features because $R_{t}^{S}$ concentrates on smaller ROI than
  $R_{t}^{L}$}.
\end{comment}

We use both fHoG and color-naming features for the scale filter, $R_{s}$. 

{\it Burak: The following sentence was NOT clear. Rewrite. Look at
  what I wrote about the translation filter, $R_{t}^{L}$.}  since the
lower template resolution (64 x n) assigned to $R_{s}$ leads to less
discriminative shape features as in Figure~\ref{fig:Filters}.c.

Smaller template size is assigned to $R_{s}$ as it estimates the scale
by correlating the filter with multiple ROIs as shown in
alg.~\ref{alg:MKCF}.  {\it Burak: Try to minimize such a
  BACK-reference. No one like looking back the parts of a paper you
  already read!}


This way, we can keep the run-time complexity of the $R_{S}$ similar
to $R_{t}^{L}$ and $R_{t}^{S}$ that only run correlation filter on a
single ROI. {\it Burak: Why do we need to keep the run-time complexity
  of $R_{S}$ and $R_{t}^{L}$ the same?}


By doing so, we can avoid fluctuating run-time performance among KCFs
which is not desired in embedded systems. {\it Burak: By doing what,
  you can avoid fluctuating run-time performance? Rewrite this
  sentence in connection with the above-mentioned!}
...
{\it Burak: What happened to the experiment of DNN features? You'll
  have to preview whatever the readers will read below this section!}
...
$5\%$ on embedded systems with GPUs.  To further improve the
performance, we employed deeper features to the scale filter as it
learns smaller ROI than the translation filters. {\it Burak: We
  employed deeper features and then what happened?}
